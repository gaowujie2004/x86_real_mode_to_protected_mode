;任务：对1.asm修改，只开放RTC中断信号，屏蔽其他中断源，这样一来 hlt 只能被RTC唤醒了。
...
SECTION MBR_CODE vstart=0
    ...
    ;4.设置8259A中断屏蔽寄存器，只开放RTC中断信号
    ;0是开放，1是关闭该中断引脚
    mov al, 0b1111_1110                         ;TODO-Tips：1-7位设置1屏蔽，开放0位即从片的IR0引脚
    out 0xa1, al                                ;TODO-Tips：0xa1是从片的IMR（中断屏蔽寄存器）端口

    mov al, 0b1111_1011                         ;TODO-Tips：默认情况下，主片上的IR2（即位2）是从片的输出引脚，应该开放，因为间接控制RTC
    out 0x21, al                                ;TODO-TIps：0x21是主片的IMR（中断屏蔽寄存器）端口
