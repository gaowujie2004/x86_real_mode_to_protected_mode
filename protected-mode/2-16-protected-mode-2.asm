; 目标：进入16位保护模式，读取超过1MB的内容（偏移量超过16位）
; 什么是16位保护模式？即内存寻址也是通过描述符高速缓存器的基地址+偏移量，需要GDT。但是CS描述符高速缓存器中的D位=0，即默认操作尺寸是16位。


; es 描述符，段基地址：0x00000000， 段界限: x0fffff， 粒度：4kb
...
SECTION MBR_CODE
    ...
    ;#1号，数据段。其实是代码，但是我想看看它的二进制数字。段基地址=0， 目标物理地址：0xffff_fff0
    mov dword [bx+0x08], 0x0000_ffff
    mov dword [bx+0x0c], 0x008f_9200  ;D位=0或者1，无影响
